...
pub async fn module_insert_or_update_on_change(
    ...
    match module {
        ...
        Module::OAuth2App => OAuth2App::on_change(data).await,
...
pub async fn module_insert_or_update_next_step(
    ...
    match module {
        ...
        Module::OAuth2App => OAuth2App::next_step(data).await,
...
pub async fn read_row_list_for_module(
    ...
    let (data, total_items) = match module {
        ...
        Module::AIIntegration => {
            let (rows, total_items) = AIIntegration::read_all(limit, offset).await?;
        ...
        // }
        Module::OAuth2App => {
            let (rows, total_items) = OAuth2App::read_all(limit, offset).await?;
            (ModuleDataList::OAuth2App(rows), total_items)
        }
        Module::EmailAccount => {
            let (rows, total_items) = EmailAccount::read_all(limit, offset).await?;
            (ModuleDataList::EmailAccount(rows), total_items)
        }
        Module::Mailbox => {
            let (rows, total_items) = Mailbox::read_all(limit, offset).await?;
...
pub async fn read_row_list_for_module_with_filter(
    ...
    let (data, total_items) = match filters {
        ModuleFilters::AIIntegration(x) => {
            let (rows, total_items) = AIIntegration::read_with_filter(x, limit, offset).await?;
            (ModuleDataList::AIIntegration(rows), total_items)
        }
...
pub async fn insert_helper(
    ...
    match data {
        ...
        ModuleDataCreateUpdate::AIIntegration(x) => x.insert_module_data().await,
